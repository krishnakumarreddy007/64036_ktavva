...
```{r}
transactVolumeByHour = OR %>% 
  group_by(New_Invoice_Hour) %>% summarise(TransactionVolume =n())

transactVolumeByHour = transactVolumeByHour %>% 
  arrange(New_Invoice_Hour) %>%
  mutate(TwoConsecutiveHourTraffic=TransactionVolume+lead(TransactionVolume),NextHour=lead(New_Invoice_Hour)) %>%
  filter(New_Invoice_Hour>=7,New_Invoice_Hour<19)

transactVolumeByHour %>% 
  filter(TwoConsecutiveHourTraffic== min(transactVolumeByHour$TwoConsecutiveHourTraffic))

#transactVolumeByHour = OR %>% group_by(New_Invoice_Hour) %>% summarise(TransactionVolume =n()) %>% arrange(New_Invoice_Hour) %>% mutate(TwoConsecutiveHourTraffic=TransactionVolume+lead(TransactionVolume), NextHour=lead(New_Invoice_Hour)) %>% filter(New_Invoice_Hour>=7,New_Invoice_Hour<19,TwoConsecutiveHourTraffic== min(transactVolumeByHour$TwoConsecutiveHourTraffic))
```
